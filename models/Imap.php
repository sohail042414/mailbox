...
class Imap
{
  ...
  public function getMessage($messageId)
  {
    ...
    if ($details) {
      ...
      $message = array(
        ...
        'body_type' => $this->getBodyType($messageId),
      );
    } else {
  ...
  /**
   * Returns an associative array with email ids and uids
   * messages in the active $folder for 
   *
   * @return Associative array with message id as key and uid as value
   */

  public function getIdsSinceDays($days)
  {

    $this->tickle();
  
      //$on_date = date("j F Y",strtotime('2018-12-07'));
    $date = date("d M Y", strtotime('-' . (int)$days . ' Day'));
      //$messages =imap_search($this->mailbox, 'SINCE "8 August 2018"');
    $ids = imap_search($this->mailbox, 'SINCE "' . $date . '"');

    $data = array();

    foreach ($ids as $id) {
      $data[$id] = imap_uid($this->mailbox, $id);
    }
  ...
  public function decode7Bit($text)
  {
      ...
      //return back, text is in correct format. 
    //return $text; 
